...
/*
  Tools and utility functions for extracting data from a byte array.
*/
uint32_t msg_extract_uint32(const byte *msg, uint8_t start, bool leftOrdering);

uint64_t msg_extract_uint64(const byte *msg, uint8_t start, bool leftOrdering);
...
uint8_t msg_insert_uint(byte (&msg)[S], T number, uint8_t start, uint8_t msgSize, bool leftOrdering = true)
{
  /*
    Error codes:
...
*/
template <typename T, std::size_t S>
uint8_t msg_insert_int(byte (&msg)[S], T number, uint8_t start, uint8_t msgSize, bool leftOrdering = true)
{
  /*
    Error codes:
      0 - Number inserted correctly
